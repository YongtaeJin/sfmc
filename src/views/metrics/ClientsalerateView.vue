<template>
    <v-container fluid>        
        <v-toolbar height="40px" background-color="primary" dark>
            <v-toolbar-title>고객별 매출 분석</v-toolbar-title>
            <v-spacer/>           
...
export default {
    ...
    methods: {
        ...
        async view() {    
            ...
            this.headItem1 = await this.$axios.post(`/api/metrics/getClientsalerate`, this.form);
            this.headItem2 = await this.$axios.post(`/api/metrics/getClientsaleratedt`, this.form);
        },
        calPer(item) {
            return ((item.a_orderamt / this.sum * 100).toFixed(2))
        },
        rowSelectHead1 :function (item, row) {                              
            if ( this.headItem1Info.c_vend == item.c_vend ) return;
